...
yandere_api_url = "https://yande.re/post/show/{}"
...
def get_wallpaper_info(id):
  ...
  url = yandere_api_url.format(id, yandere_api_url)
  ...
  if content_tags:
    for li in content_tags.find_all("li"):
      tag_data = li.find("a").get("href")
      tag_data = tag_data.split("=")[1]
      parsed_tags.append(tag_data)
  else:
    pass
...
def main():
  ...
  print("""This script will scan every file named: yandere_<id>.<ext>
and scrape yande.re to get tags and purity.
then put these, separated by newlines, in a sidecar file named the same as original, but with a txt extension at the end, 
so for example: yandere_123456.png.txt <- the sidecar file
        """)
